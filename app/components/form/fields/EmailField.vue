 ...
 * Email input with HTML5 validation and auto-lowercase on blur.
 */
...
function handleBlur() {
  ...
  if (inputValue.value) {
    emit('update:modelValue', inputValue.value.toLowerCase().trim())
  }
...
<template>
  <div class="form-field" :class="{ 'has-error': hasErrors, 'is-disabled': disabled }">
    ...
    <InputText
      ...
      v-model="inputValue"
      type="email"
      :placeholder="component.placeholder || 'you@example.com'"
